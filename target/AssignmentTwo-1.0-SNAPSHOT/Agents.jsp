...
<div class="container">
    <div class="row marginTop">
        ...
        <c:if test="${fn:length(agentList) <= 0}">
            <br><h1>There are no agents</h1>
        </c:if>
        <c:if test="${fn:length(agentList) > 0}">
            <br><h1>List of all agents (<c:out value="${fn:length(agentList)}"/>)</h1>
            <table class="table marginTop">
                ...
                <c:forEach items="${agentList}" var="agent">
                    <tr class="table-active">
                        ...
                        <td><c:out value="${agent.fax}"/></td>
                        <td><c:out value="${agent.email}"/></td>
                        <td><a href="<c:url value="GetAgentDetailsServlet?agentID=${agent.agentId}"/>" class="btn btn-success">Edit</a></td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
